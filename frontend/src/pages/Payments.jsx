...
import ConsolidatedLedgerTab from './ConsolidatedLedgerTab';
...
export default function Payments() {
  ...
  // handleInlineCellSave is called from ConsolidatedLedgerTab with a local setExpandedPanelPayments
  const handleInlineCellSave = async (payment, field, newValue, expandedPanelPayments, setExpandedPanelPayments) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (refreshedData.success && setExpandedPanelPayments) {
          setExpandedPanelPayments(refreshedData.payments || []);
        }
        fetchPaymentsAndPanels(currentPage, true);
      }
  ...
  const fetchPaymentsAndPanels = async (page = currentPage, isSilent = false) => {
    try {
      ...
      let url = `/payments?page=${page}&limit=${pageSize}`;
  ...
  const handleSavePayment = async (e) => {
    ...
    try {
      ...
      if (modalMode === 'receive') {
        ...
        // Strict Validation: If there are unpaid bills, the admin must allocate the payment to them first
        const totalPendingDues = unpaidBills.reduce((acc, b) => {
          const remaining = (b.billAmount - (b.billDiscount || 0)) - b.paidAmount;
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'list' ? (
        <TransactionLedgerTab
          ...
          userRole={userRole}
          FALLBACK_PAYMENT_TYPES={FALLBACK_PAYMENT_TYPES}
          pageSize={pageSize}
          setPageSize={(newSize) => {
            setPageSize(newSize);
            setCurrentPage(1);
        ...
        <ConsolidatedLedgerTab
          panels={panels}
          userRole={userRole}
          setViewingPayment={setViewingPayment}
          setSelectedReceiptPayment={setSelectedReceiptPayment}
          handleDeletePaymentClick={handleDeletePaymentClick}
          handleInlineCellSave={handleInlineCellSave}
          getLast30DaysData={getLast30DaysData}
        />
      )}
                ...
                <div className="flex items-center gap-3 mt-4">
                  <button
                    ...
                    className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border transition-all text-xs font-bold shadow-sm ${showPaymentDiscount
                      ? 'border-rose-300 dark:border-rose-800 bg-rose-50/50 dark:bg-rose-950/20 text-rose-700 dark:text-rose-400'
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3 animate-in slide-in-from-top-1">
                      <div>
                        ...
                        <input
                          ...
                          placeholder="e.g. 500"
                          className="w-full rounded-xl px-4 py-3 text-sm glass-input text-rose-600 dark:text-rose-400 text-base font-mono"
                          min="0"
                        />
                      </div>
                    </div>
  ...
  `}
      ...
      {viewingPayment && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-2xl rounded-3xl bg-white dark:bg-slate-900 p-6 md:p-8 border border-slate-200 dark:border-slate-800 shadow-2xl z-10 animate-in fade-in zoom-in-95 duration-200 text-slate-900 dark:text-slate-100">
            <button
            ...
            {/* Header */}
            <div className="flex items-center gap-3.5 mb-6 border-b border-slate-100 dark:border-slate-800 pb-5">
            ...
            <div className="space-y-6 overflow-y-auto max-h-[60vh] pr-1 scrollbar-thin">
              {/* Panel Client Info */}
              <div className="bg-white dark:bg-slate-950/40 border border-slate-200/80 dark:border-slate-800/80 border-l-4 border-l-indigo-600 rounded-2xl p-5 space-y-4 shadow-sm">
                <div className="text-xs font-bold uppercase tracking-wider text-indigo-600 dark:text-indigo-400">Panel Client Details</div>
                <div className="grid grid-cols-2 gap-4 text-xs">
                  <div>
                    <span className="text-slate-500 block mb-1 font-medium">Panel Name</span>
                    <span className="text-slate-800 dark:text-slate-200 text-sm font-semibold">{viewingPayment.panelId?.panelName || 'N/A'}</span>
                  </div>
                  <div>
                    <span className="text-slate-500 block mb-1 font-medium">Owner Name</span>
                    <span className="text-slate-800 dark:text-slate-200 text-sm font-semibold">{viewingPayment.panelId?.ownerName || 'N/A'}</span>
                  </div>
                  <div>
              ...
              <div className="bg-white dark:bg-slate-950/40 border border-slate-200/80 dark:border-slate-800/80 border-l-4 border-l-emerald-600 rounded-2xl p-5 space-y-4 shadow-sm">
                <div className="text-xs font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400">Financial Ledger Details</div>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-xs">
                  ...
                  {viewingPayment.billAmount > 0 && (
                    <>
                      <div>
                        ...
                        <span className="text-slate-800 dark:text-slate-200 font-semibold font-mono">{viewingPayment.quantity !== undefined && viewingPayment.quantity !== null ? viewingPayment.quantity : 1}</span>
                      </div>
                      <div>
                        <span className="text-slate-500 block mb-1 font-medium">Unit Price</span>
                        <span className="text-slate-800 dark:text-slate-200 font-semibold font-mono">₹{viewingPayment.unitPrice?.toLocaleString() || '0'}</span>
                      </div>
                      <div>
                  ...
                  {viewingPayment.amountReceived > 0 && (
                    <div>
                      ...
                      <span className="text-slate-800 dark:text-slate-200 font-semibold">{viewingPayment.paymentMode}</span>
                    </div>
                  )}
                  {viewingPayment.amountReceived > 0 && (
                    <div>
                      <span className="text-slate-500 block mb-1 font-medium">Bank Name</span>
                      <span className="font-semibold text-slate-800 dark:text-slate-200">{viewingPayment.bankName || 'N/A'}</span>
                    </div>
                  )}
              ...
              {/* Added By & Remarks */}
              <div className="bg-white dark:bg-slate-950/40 border border-slate-200/80 dark:border-slate-800/80 border-l-4 border-l-amber-500 rounded-2xl p-5 space-y-4 shadow-sm">
                <div className="text-xs font-bold uppercase tracking-wider text-amber-600 dark:text-amber-400">Audit & Comments</div>
                <div className="space-y-3 text-xs">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-slate-800 dark:text-slate-200 font-semibold">{viewingPayment.addedBy?.name || 'Staff User'}</span>
                      <span className="text-[10px] text-slate-500 dark:text-slate-500 font-medium">({viewingPayment.addedBy?.email || 'System Staff'})</span>
                    </div>
                  </div>
                  <div>
                    <span className="text-slate-500 font-medium block mb-1.5">Remarks / Remarks Description:</span>
                    <div className="bg-slate-50/60 dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-xl p-4 text-slate-700 dark:text-slate-300 font-medium italic leading-relaxed">
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-800">
                      ...
                      <div className="space-y-2 max-h-36 overflow-y-auto pr-1 scrollbar-thin">
                        {viewingPayment.editHistory.map((history, idx) => (
                          <div key={idx} className="bg-slate-50/60 dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-xl p-3.5 text-xs space-y-1.5">
                            <div className="flex items-center justify-between text-[10px]">
                              <span className="text-amber-700 dark:text-amber-400 font-semibold">Edited By: {history.editedBy?.name || 'Staff Admin'}</span>
            ...
            {/* Footer buttons */}
            <div className="flex items-center justify-end gap-4 mt-6 pt-5 border-t border-slate-200 dark:border-slate-800">
              ...
              <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 text-sm font-semibold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/30 rounded-lg hover:bg-amber-500 hover:text-white hover:border-transparent transition-all duration-200 active:scale-95 shadow-sm"
