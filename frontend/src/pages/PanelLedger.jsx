          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2 text-xs">
              ...
              <div className="flex items-center gap-2 text-slate-700">
                ...
                <span className="truncate font-medium text-slate-650" title={panel?.ownerEmail}>{panel?.ownerEmail}</span>
              </div>
              <div className="flex items-center gap-2 text-slate-700">
                <Phone className="h-3.5 w-3.5 text-slate-400 shrink-0" />
                <span className="font-medium text-slate-655 text-slate-700">{panel?.phoneNumber}</span>
              </div>
            </div>
          </div>
          ...
          <div className="overflow-x-auto p-4 bg-slate-50">
            <div className="flex items-center justify-between mb-3 text-[11px] font-mono">
              <div className="flex items-center gap-3">
                ...
                <span className="text-slate-500 font-bold">Transaction Ledger</span>
              </div>
              ...
              <tbody>
                {getLast30DaysData().map((row, idx) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-1.5 text-slate-650 text-slate-700 truncate max-w-xs font-semibold" title={`${row.remark} (by ${row.addedBy})`}>
                      {row.remark !== '-' ? (
                        <span className="flex items-center gap-1.5">
                          <span className="text-slate-800">{row.remark}</span>
                          <span className="text-[10px] text-slate-400 font-bold bg-slate-100 border border-slate-150 px-1 py-0.2 rounded shrink-0">({row.addedBy})</span>
                        </span>
                      ) : '-'}
                    </td>
                  </tr>
                ...
                <tr className="bg-slate-150 bg-slate-100 border-t-2 border-slate-300 font-bold text-slate-900 text-xs">
                  <td className="border-r border-slate-200 text-center text-slate-450 bg-slate-100 py-2.5 font-bold">
              ...
              <tbody>
                {getLast30DaysData().map((row, idx) => (
                  ...
                  >
                    ...
                    <td className="border-r border-slate-200 px-4 py-2 text-slate-650 text-slate-700 w-28 font-medium">
                      {row.displayDate}
                    </td>
                    ...
                    <td className="px-4 py-2 text-slate-650 text-slate-700 truncate max-w-sm font-semibold" title={`${row.remark} (by ${row.addedBy})`}>
                      {row.remark !== '-' ? (
                        <span className="flex items-center gap-1.5">
                          <span className="text-slate-800">{row.remark}</span>
                          <span className="text-xs text-slate-450 text-slate-400 font-bold bg-slate-100 border border-slate-150 px-1.5 py-0.2 rounded shrink-0">({row.addedBy})</span>
                        </span>
                      ) : '-'}
                    </td>
                  </tr>
